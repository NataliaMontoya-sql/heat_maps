...
elif pagina_actual == "📅 Comparativa Histórica" and df is not None:
    st.header("📆 Análisis Temporal")
    año = st.selectbox("Seleccionar Año", df['YEAR'].unique())
    df_anual = df.groupby('YEAR')[['ALLSKY_KT', 'ALLSKY_SFC_SW_DWN']].mean().reset_index()
    fig_line = px.line(df_anual, x='YEAR', y=['ALLSKY_KT', 'ALLSKY_SFC_SW_DWN'], markers=True)
    st.plotly_chart(fig_line, use_container_width=True)

elif pagina_actual == "📦 Datos" and df is not None:
    st.header("📂 Conjunto de Datos Completo")
    min_rad = st.slider("Radiación Mínima", df['ALLSKY_SFC_SW_DWN'].min(), df['ALLSKY_SFC_SW_DWN'].max(), 150.0)
    max_kt = st.slider("Claridad Máxima", df['ALLSKY_KT'].min(), df['ALLSKY_KT'].max(), 95.0)
